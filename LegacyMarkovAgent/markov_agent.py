...
class MarkovAgent():
  ...
  def state_value(self, action, state):
    ...
    if len(self.reward_obs.e_prob[state]) == state:
      self.reward_obs.fit_emission_prob(np.concatenate(self.all_rewards), np.concatenate(self.all_obs))
    expected_rew = np.random.choice(np.arange(0, len(self.reward_obs.e_prob[state])),
                                                                      p=self.reward_obs.e_prob[state])
    expected_rew *= np.argmin(self.reward_obs.e_prob[state]) # Greatest reward given state

    expected_rew *= self.obs_to_obs.expected_consecutive_obs(state)
    # Expected value
    #for col in range(len(self.reward_obs.e_prob[most_likely_obs])):
    # expected_rew += (col + 1) * self.reward_obs.e_prob[most_likely_obs][col]

    # Penalize recent action
  ...
  def act(self, obs, time_t =0, training=True):
    if training:
      ...
      action_reward = []
      obs_loc = self.clean_observation(obs, one=True)
      
      for action in range(2):
        state_next = self.observe_next_state(action)
        
        expected_rew = self.state_value(action, state_next)

        #import pdb; pdb.set_trace()
        
        expected_rew += self.state_value(action, obs_loc[0])
      ...
      if sum(action_reward) == 0:
        if self.all_actions[-1] == 0:
          action = 1# int(np.random.randint(0, 11) < 8)
        else:
          action = 0 #int(np.random.randint(0, 11) > 8)
      else:
  ...
  def clean_observation(self, obs, one=False):
    '''
    observation                 Min         Max
    0	Cart Position             -4.8            4.8
    1	Cart Velocity             -Inf            Inf
    ...
    if not one: # Observation
      return self.sigmoid_array(np.array(obs)[:,0])

    
    return self.sigmoid_array(np.array(obs))
  ...
  def terminal(self, obs, reward, acts=[]):
    ...
    cleaned_obs = self.clean_observation(obs)
    ...
    self.act_to_act.fit_transition_prob(loc_acts, max(loc_acts) + 1, 'reverse')
